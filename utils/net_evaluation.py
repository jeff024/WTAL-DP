...
def print_map(map):
    class_name_list = ['BaseballPitch', 'BasketballDunk', 'Billiards', 'CleanAndJerk',
    ...
    map = pd.DataFrame(map, columns=class_name_list, index=np.round(np.arange(0.1,1.0,0.10), 1))
    map = map.transpose()

    def style_negative(v, props=''):
        return props if v < 0.1 else None
    s2 = map.style.applymap(style_negative, props='color:red;')
    print(map.to_string())
...
def get_proposal_oic(tempseg_list, int_temp_scores, c_pred, c_pred_scores, t_factor, lamb=0.25, gamma=0.10): # [0.25, 0.20]
    temp = []
...
class ANETDetection(object):
    ...
    def wrapper_compute_average_precision(self):
        ...
        # try:
        #     print_map(ap)
        #     with open("map_result.json", 'w') as fp:
        #         json.dump(ap.tolist(), fp)
        # except:
        #     pass
    ...
    def evaluate(self):
        ...
        print("-------------------------------------------------------------------------------")
        if self.subset == "val":
            print('|mAP@0.5: {:.4f}'.format(self.mAP[0]))
            print("-------------------------------------------------------------------------------")
        else:
            print('|Average-mAP@0.1:0.5: {:.4f}'.format(self.mAP[:5].mean()))
            print("-------------------------------------------------------------------------------")
            print('|Average-mAP@0.1:0.7: {:.4f}'.format(self.mAP[:7].mean()))
            print("-------------------------------------------------------------------------------")
